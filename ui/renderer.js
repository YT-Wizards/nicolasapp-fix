...
const translations = {
  es: {
    ...
    addHeygenFirst: 'Añade primero el vídeo completo de HeyGen.', enterTitle: 'Escribe el título que tendrá el archivo final.', chooseQr: 'Selecciona la tarjeta completa del QR.', checkingTest: 'Comprobando conexiones y preparando la prueba de 90 s…', checkingFull: 'Comprobando conexiones y preparando el vídeo completo…', testQueued: 'Prueba puesta en cola', pasteScriptError: 'Pega el guion final antes de continuar.', chooseAudioError: 'Selecciona la locución final antes de continuar.', planning: 'Transcribiendo el audio y creando los prompts…', promptsReady: '{count} prompts listos.', promptsReadyHelp: 'Cada uno tiene un número y el tiempo exacto de montaje.', promptsFallback: 'No se pudo reescribir con IA; se usaron prompts locales de respaldo. Revisa las advertencias antes de generar clips.', downloadPrompts: 'Descargar prompts (.txt)', audioReady: 'audio final listo', folderReady: 'Carpeta seleccionada · ahora comprueba los clips', createPromptsFirst: 'Crea primero los prompts.', chooseFolder: 'Selecciona la carpeta de clips.', checkingClips: 'Comprobando nombres y duraciones…', allReady: 'Todo listo.', allReadyHelp: '{count} clips están presentes y tienen la duración necesaria.', cannotAssemble: 'Aún no se puede montar.', missing: 'Faltan: {numbers}', tooShort: 'Demasiado cortos: {clips}', enterFinalTitle: 'Escribe un título para el vídeo final.', completeSteps: 'Completa los pasos anteriores antes de montar.', queueAssembly: 'Añadiendo el montaje a Producción…', assemblyQueued: 'Montaje puesto en cola', resumeActive: 'Este vídeo ya está en producción', resumeQueued: 'Continuación puesta en cola desde el checkpoint', loadScript: 'Cargar archivo'
  },
  en: {
...
function applyLanguage(language) {
  ...
  set('#externalScriptButton', 'loadScript');
...
async function selectExternalScript() {
  const script = await window.vyt.chooseScript();
  if (!script) return;
  $('#externalScriptInput').value = script.text;
  if (!$('#externalTitleInput').value.trim()) $('#externalTitleInput').value = script.name.replace(/\.[^.]+$/, '');
  showError($('#externalError'), '');
}
...
async function createExternalPlan() {
  try {
    ...
    const fallbackWarning = result.plan.prompt_generation?.mode === 'fallback' ? `<br><span class="warning">${t('promptsFallback')}</span>` : '';
    $('#externalPlanSummary').innerHTML = `<b>${tr('promptsReady', { count: result.plan.scenes.length })}</b> ${t('promptsReadyHelp')} ${fallbackWarning} <a href="#" id="downloadExternalPrompts">${t('downloadPrompts')}</a>`;
...
$('#externalScriptButton').onclick = selectExternalScript;

...
PHONE DIRECTION CHECK: When the narration or intended subject says that a person is looking at, reading, checking, holding or operating a phone, the person's gaze and body orientation must be directed at the device and the display must face that person. Use a side, over-the-shoulder or three-quarter composition: the display must not face the camera head-on and must be turned away, edge-on or visible only from a natural oblique angle. Reject a frontal screen-first composition even when the person looks down at the device. A screen may face the viewer only when the narration explicitly requires showing the screen to the viewer or another person.
...
PHONE DIRECTION CHECK: When the narration or intended subject says that a person is looking at, reading, checking, holding or operating a phone, the person's gaze and body orientation must be directed at the device and the display must face that person. Use a side, over-the-shoulder or three-quarter composition: the display must not face the camera head-on and must be turned away, edge-on or visible only from a natural oblique angle. Reject the clip when the display is frontal to the camera, even if the person looks down at the phone. A screen may face the viewer only when the narration explicitly requires showing the screen to the viewer or another person.
    ...
    if not isinstance(contract, dict) and re.search(r"\b(phone|smartphone|telephone|teléfono|telefono|móvil|movil)\b", full_content):
        ...
        if re.search(
            ...
            rules.append(
                ...
                "The display must be turned away, edge-on or visible only from a natural oblique angle; it must not face "
                "the camera head-on. Use a side or over-the-shoulder composition. Reject a frontal screen-first pose even "
                "when the person looks down at the device. A screen may face the viewer only when showing it is explicitly narrated."
            )
    return "\n".join(rules)
